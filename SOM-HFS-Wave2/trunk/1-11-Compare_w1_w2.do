...
* keep overlapping regions
keep if !mi(ind_profile) 
save "${gsdTemp}/hh_w1w2_comparison.dta", replace



* 3- Analysis outputs, indicator by indicator
use "${gsdTemp}/hh_w1w2_comparison.dta", clear
* Set worksheets
foreach r in Mogadishu NE-Urban NW-Urban NE-Rural NW-Rural {
	di "`r'"
	preserve
	...
	* focus on adult literacy
	...
	sort n
	drop n
	export excel using "${gsdOutput}/W1W2-comparison_v1.xlsx", sheet("`r'") sheetmodify cell(B3) first(varlabels)
	restore
}
