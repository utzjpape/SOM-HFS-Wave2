*Impute complete consumption (using an Adult Equivalent scale from OECD)
...
merge 1:m strata ea block hh using "${gsdData}/1-CleanTemp/hhm.dta", assert(match) nogen keepusing(no_children no_adults)
duplicates drop
...
keep ind_profile region astrata strata ea block hh hhsize weight opt_mod pchild psenior hhempl hhsex hhedu hh_type hh_drinkwater hh_floor hh_ownership hh_hunger remit12m cons_f? cons_nf? cons_d hh_ptype cons_all_* no_children
drop if weight>=.
...
*Convert household size to an Adult Equivalent (AE) measure using the OECD method
gen no_adult=hhsize-no_children
gen hhsize_ae=1 if hhsize==1
replace hhsize_ae=1 + (0.7*(no_adult -1 )) + (0.5*no_child) if hhsize>1
rename hhsize hhsize_original
rename hhsize_ae hhsize

*Adjust to per AE per day 
forvalues i = 0/4 {
...
********************************************************************
* Change aggregation 
local model = "hhsize_original pchild psenior i.hhsex i.hhempl hhedu i.hh_type i.hh_drinkwater i.hh_floor i.hh_ownership i.ind_profile_impute rural i.hh_hunger i.remit12m"
local model = "i.pmi_cons_f0 i.pmi_cons_nf0 i.pmi_cons_d `model'"
*Create core consumption for comparison
save "${gsdData}/1-CleanTemp/mi-pre_ae.dta", replace
*Run imputation
use "${gsdData}/1-CleanTemp/mi-pre_ae.dta", clear
xtset, clear
mi set wide
...
save "${gsdData}/1-CleanTemp/mi-analysis_ae.dta", replace


********************************************************************
*Extract the 100 imputations
********************************************************************
use "${gsdData}/1-CleanTemp/mi-analysis_ae.dta", clear
forvalues i = 1/`n' {
	use "${gsdData}/1-CleanTemp/mi-analysis_ae.dta", clear
	mi extract `i', clear	
	gen mi = `i'
	save "${gsdTemp}/mi_`i'_ae.dta", replace
}
*append
clear
forvalues i = 1/`n' {
	append using "${gsdTemp}/mi_`i'_ae.dta"
}
save "${gsdTemp}/mi-extract_ae.dta", replace
*Analysis on extract dataset
use "${gsdTemp}/mi-extract_ae.dta", clear
fastgini tc_imp [pweight=hhweight]
collapse (mean) tc_* mi_cons_f? mi_cons_nf? mi_cons_d (mean) poorPPP_prob = poorPPP poorPPP125_prob = poorPPP125 poorPPP_vulnerable_10_prob = poorPPP_vulnerable_10 poorPPP_vulnerable_20_prob = poorPPP_vulnerable_20, by(strata ea block hh hhsize_original weight hhweight opt_mod plinePPP plinePPP125)
...
rename poorPPP_prob poorPPP_prob_AE
save "${gsdData}/1-CleanTemp/hhq-poverty_ae.dta", replace
