...
********************************************************************
* I. Cleaning including rural PL and urban JB
********************************************************************
...
drop mod_opt
save "${gsdData}/1-CleanTemp/hh_fcons_all.dta", replace
...
********************************************************************
* II. Cleaning excluding rural PL and urban JB
********************************************************************
...
use "${gsdData}/1-CleanInput/food.dta", clear
drop if inlist(strata,31,33,38,40,42)
merge m:1 strata ea block hh using "${gsdData}/1-CleanInput/hh.dta", keep(match) nogen keepusing(weight mod_opt)
order weight, after(hh)
drop cons_q_kdk ownprod pr_kdk free free_q free_main
rename mod_item opt_mod 
...
*Include relevant info to differentiate between zero and items not administered
drop region weight enum mod_opt opt_mod
merge m:1 strata ea block hh using "${gsdData}/1-CleanInput/hh.dta", keep(match) nogen keepusing(region weight enum mod_opt)
drop if inlist(strata,31,33,38,40,42)
merge m:m foodid using "${gsdData}/1-CleanInput/food.dta", nogen keep(master match) keepusing(mod_item)
...
*Include the key variables for all the records
drop region weight enum mod_opt mod_item astrata
merge m:1 strata ea block hh using "${gsdData}/1-CleanInput/hh.dta", keep(match using) nogen keepusing(region weight enum mod_opt astrata)
merge m:m foodid using "${gsdData}/1-CleanInput/food.dta", nogen keep(master match) keepusing(mod_item)
rename foodid itemid
order region strata ea block hh enum weight mod_opt mod_item
duplicates drop region strata ea block hh mod_opt itemid, force
preserve
drop if inlist(strata,31,33,38,40,42)
save "${gsdData}/1-CleanTemp/food.dta", replace
restore
*Now the data is collapsed at the household level and converted into wide format 
collapse (sum) cons_usd, by(astrata strata ea block hh mod_opt mod_item)
replace mod_item=0 if mod_item==.
reshape wide cons_usd, i(astrata strata ea block hh mod_opt) j(mod_item)
ren cons_usd* cons_f* 
*Then we includes zero values for optional modules without consumption and correct naming of missing values
forval i=1/4 {
...
*Exclude rural PL and urban JB
forval i=0/4 {
	replace cons_f`i'=. if inlist(strata,31,33,38,40,42)
}
drop mod_opt
